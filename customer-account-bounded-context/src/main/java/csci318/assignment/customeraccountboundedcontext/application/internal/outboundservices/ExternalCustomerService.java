...
public class ExternalCustomerService {
    ...
    public List<ProductOrderResponseDTO> getCustomerOrderHistory(List<String> orderIds) {
        final String url = "http://localhost:8082/order/internal/";
        List<ProductOrderResponseDTO> orders = new ArrayList<>();
        for (String id : orderIds) {
            orders.add(restTemplate.getForObject(url + id, ProductOrderResponseDTO.class));
        }
        return orders;
    }
}
